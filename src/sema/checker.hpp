...
private:
    ...
    shape::DimContext& dims_ = model_->dims;
    TypeStore& types_ = model_->types;
    std::deque<Entity>& entities_ = model_->entities;
    std::map<EntityId, DeclInfo>& decls_ = model_->decls;
    ...
    ast::ExprId current_expr_ = ast::no_id; // the expression being checked, for facts
    Env* env_ = nullptr;
