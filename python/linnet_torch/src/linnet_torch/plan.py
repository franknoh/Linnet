...
class Env:
    ...
    def dtype(self, spec: str | dict[str, Any]) -> torch.dtype:
        if isinstance(spec, str):
            return TORCH_DTYPES[spec]
        var = int(spec["var"])
        if var not in self.dtypes:
            raise PlanError(f"dtype `{spec['name']}` is not bound")
        return TORCH_DTYPES[self.dtypes[var]]

    def dtype_name(self, spec: str | dict[str, Any]) -> str:
        if isinstance(spec, str):
            return spec
        return self.dtypes[int(spec["var"])]
...
def find_compiler() -> str:
    ...
    if found is None:
        raise PlanError("cannot find the `linnet` executable; set LINNET_BIN or add it to PATH")
    return found
...
def compile_plan(
    ...
    if std_root is not None:
        command += ["--std", str(std_root)]
    command.append(str(source))
    completed = subprocess.run(command, capture_output=True, text=True, check=False)
    if completed.returncode != 0:
        raise PlanError(completed.stderr.strip() or f"`linnet plan` failed for {source}")
    return Plan.from_json(completed.stdout)
