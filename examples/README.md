...
| `05-llama` | A Llama-style decoder as a multi-module package: `crate.` imports, a `pub const` base frequency, rotary tables computed at compile time from `iota`, grouped-query attention whose `where` clause states the divisibility the reshapes rely on, `broadcast_to` for repeating key/value heads, six entries (`forward`; `next_token`, which slices the last position with `S - 1`; `decode`, which generates one token at a time from per-layer KV caches held in `state` members and written with a masked `select` at the runtime position; `generate<Steps>`, greedy decoding inside the graph with `static for i in 0..Steps` and `std.nn.decoding::argmax`; `sample<Steps>`, keyed sampling with `std.random`; and `generate_until<MaxNew>`, a runtime `while` loop that stops once every row has produced the end token), and a dtype generic defaulting to `bf16`. |
