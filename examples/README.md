...
| `05-llama` | A Llama-style decoder as a multi-module package: `crate.` imports, a `pub const` base frequency, rotary tables computed at compile time from `iota`, grouped-query attention whose `where` clause states the divisibility the reshapes rely on, `broadcast_to` for repeating key/value heads, three entries (`forward`; `next_token`, which slices the last position with `S - 1`; and `decode`, which generates one token at a time from per-layer KV caches held in `state` members and written with a masked `select` at the runtime position), and a dtype generic defaulting to `bf16`. |
